 ...
 */
  ...
  do {
	...
	if(selection==1){				/* next-fit */

		if(next_selected==NIL_HOLE){
			next_selected = hole_head;
		}
	...
	}else if(selection==2){				/* best-fit */

		struct hole *best = hole_head;
		unsigned int bestfitting = 999999;
		prev_ptr = NIL_HOLE;
		hp = hole_head;

		while(hp != NIL_HOLE && hp->h_base < swap_base){

			if((hp->h_len >= clicks)&&(hp->h_len - clicks)<bestfitting){
				best = hp;
				bestfitting = hp->h_len - clicks;
			}
			hp = hp->h_next;
			prev_ptr = hp;
		}
	...
	}else if(selection==3){				/* worst-fit */

		struct hole *worst = hole_head;
		unsigned int worstfitting = 0;
		prev_ptr = NIL_HOLE;
		hp = hole_head;

		while(hp != NIL_HOLE && hp->h_base < swap_base){

			if((hp->h_len >= clicks)&&(hp->h_len - clicks)>worstfitting){
				worst = hp;
				worstfitting = hp->h_len - clicks;
			}
			prev_ptr = hp;
			hp = hp->h_next;
		}
	...
	}else if(selection==4 || selection==5){	/* first-fit */
